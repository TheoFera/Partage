-- Bypass a potentially stale RPC execution path by publishing new function names.
-- The business logic is identical to the current strict lot-only platform invoice logic.

create or replace function public.create_platform_invoice_for_order_v2(p_order_id uuid)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  insert into public.facture_lignes (
    ...
    jsonb_build_object(
      ...
      'generator_version', 'plat_prod_v2026_03_13_trace_02_v2_rpc_name',
...
create or replace function public.create_platform_invoice_and_send_for_order_v2(p_order_id uuid)
returns jsonb
language plpgsql
security definer
set search_path to 'public'
...
declare
  v_result jsonb;
begin
  v_result := public.create_platform_invoice_for_order_v2(p_order_id);
  perform public.call_process_emails_sortants();
  return v_result;
end;
$function$;

create or replace function public.admin_create_platform_invoice_and_send_for_order_v2(p_order_id uuid)
returns jsonb
language plpgsql
security definer
set search_path to 'public'
...
begin
  ...
  v_result := public.create_platform_invoice_and_send_for_order_v2(p_order_id);
...
grant execute on function public.create_platform_invoice_for_order_v2(uuid) to authenticated, service_role;
grant execute on function public.create_platform_invoice_and_send_for_order_v2(uuid) to authenticated, service_role;
grant execute on function public.admin_create_platform_invoice_and_send_for_order_v2(uuid) to service_role;
