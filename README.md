...
Pour Stripe, configurez aussi les secrets Edge Functions `STRIPE_SECRET_KEY` (obligatoire), `STRIPE_API_BASE` (optionnel, par défaut `https://api.stripe.com`), `STRIPE_API_BASE_V2` (optionnel, par défaut `https://api.stripe.com`), `STRIPE_CHECKOUT_UI_MODE` (optionnel, par défaut `embedded_page`) et `STRIPE_CONNECTED_ACCOUNT_COUNTRY` (optionnel, par défaut `FR`).
